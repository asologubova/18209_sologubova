...
int main() {
	...
	while (res >= E){
		...
		#pragma omp parallel for schedule(dynamic, 625)
		for (i = 0; i < size; i++){
			y[i] = 0;
			for (j = 0; j < size; j++){
				y[i] += (*(A + i * size + j)) * (*(x + j));
		...
		#pragma omp parallel for reduction(+: lenY) schedule(dynamic, 625)
		for (i = 0; i < size; i++) {
			lenY += (y[i] * y[i]);
		}
		#pragma omp parallel for schedule(dynamic, 625)
		...
		#pragma omp parallel for reduction(+: scal1) schedule(dynamic, 625)
		for (i = 0; i < size; i++) {
			scal1 += (y[i] * Ay[i]);
		}
		#pragma omp single
		...
		#pragma omp parallel for reduction(+: scal2) schedule(dynamic, 625)
		for (i = 0; i < size; i++) {
			scal2 += (Ay[i] * Ay[i]);
		}
		#pragma omp single
